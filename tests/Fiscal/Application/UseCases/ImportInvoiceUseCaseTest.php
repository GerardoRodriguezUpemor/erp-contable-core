...
class ImportInvoiceUseCaseTest extends TestCase
{
    ...
    public function test_it_aborts_if_invoice_already_exists(): void
    {
    ...
    {
        $this->invoiceRepository->expects($this->once())->method('exists')->willReturn(false);

        $taxStrategy = $this->createMock(TaxStrategyInterface::class);
        $taxStrategy->expects($this->once())
            ->method('calculateTaxes')
            ->willReturn([
                new Tax('IVA', TaxCategory::TRANSFERRED, new Money(50000), 0.16) // Wrong amount — creates discrepancy
            ]);

        $this->taxFactory->expects($this->once())->method('create')->willReturn($taxStrategy);
        $this->invoiceRepository->expects($this->never())->method('save');
